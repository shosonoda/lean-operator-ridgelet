...
pinned `lean-toolchain`; the `README badges` workflow fails when they are stale, so rerun the script
...
.github/workflows/           pages.yml (blueprint on GitHub Pages), badges.yml (README badges check)
...
```sh
cd OperatorRidgeletBlueprint
./scripts/ci-pages.sh       # lake exe vbp build && lake exe vbp check, then assembles _site/
```
...
`.github/workflows/badges.yml` runs `python3 scripts/update-readme-badges.py --check` on every
push and pull request; it needs no Lean.
