...
its anti-dual by `Tempered.spectralAntiDual μ ν`, the extended transform `R_ρ` by
`Tempered.ridgeletExtension`, and the Riesz map `T_α` by `Tempered.rieszMap` (see
`OperatorRidgelet.Tempered.Defs` for these design choices).  The distributional admissibility
constant `C^{(α)}_{β,ρ}` is the shared `temperedAdmissibilityConst`.
-/
