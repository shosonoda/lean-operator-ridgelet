...
/-- The star subalgebra of `C((d → AddCircle T), ℂ)` generated by `mFourier n` for `n ∈ ℤᵈ` is in fact
...
@[simp]
theorem coe_mFourierBasis : ⇑(mFourierBasis (T := T) (d := d)) = mFourierLp 2 := HilbertBasis.coe_mk _ _
